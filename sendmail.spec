...
%description -l de
Sendmail ЭbertrДgt Mails zwischen Rechnern. Es implementiert eine
allgemeine Mail-Routing-Funktion Эber das Netzwerk mit Aliasing und
Weiterleiten von Nachrichten, automatischem Routing an
Netzwerk-Gateways und flexible Konfiguration. Wenn Sie E-Mails Эber
das Internet senden und empfangen mЖchten, brauchen Sie sendmail.

%description -l es
sendmail es un agente de transporte de correo electrСnico, que mueve
mensajes entre mАquinas. Implementa facilidades de internetwork y
rutado, caracterizando cambio de nombres (aliases) y envМo a nuevas
direcciones ( forwarding ), rutado automАtico para gateways de la red
y configuraciСn flexible. NecesitarАs del sendmail si deseas enviar y
recibir mensajes a travИs de la Internet.

%description -l fr
Sendmail est un agent de transport de courrier, qui est le programme
transfИrent le courrier d'une machine Ю l'autre. Sendmail implИmente
une facilitИ gИnИrale de routage de courrier entre les rИseaux, permet
l'\"aliasing\" et le \"forwarding\", un routage automatique sur les
passerelles du rИseau, et une configuration flexible.

%description -l pl
Sendmail jest programem umo©liwiaj╠cym wymianЙ poczty elektronicznej
miЙdzy komputerami w sieci (MTA). Zajmuje siЙ przekazywaniem poczty
elektronicznej miЙdzy bramkami pocztowymi i dostarczaniem przesyЁek na
konta docelowe. Bardzo dobrze obsЁuguje aliasy pocztowe a jego
dodatkowym atutem jest prosta konfiguracja. DziЙki rozbudowanym
...
%description -l pt_BR
O sendmail И um agente de transporte de correio eletrТnico, que move
mensagens entre mАquinas. Ele implementa facilidades de internetwork e
roteamento, caracterizando troca de nomes (aliases) e remessa a novos
endereГos ( forwarding ), roteamento automАtico para gateways da rede
e configuraГЦo flexМvel.
